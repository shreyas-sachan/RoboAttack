	...
	*/
	...
	canvas.addEventListener('click', function(event) {
		...
		for(a=0 ; a<map.Edges.length ; a++) {
			...
			var m = (y1 - y2)/(x1 - x2);
			var distance = Math.abs(x - m*y + m*y2 - x2)/Math.sqrt(1 + m*m);
	...
	function drawCoins() {
    	...
    	ctx.fillText("Coins: " + this.minCost, canvas.width-100, 40);
	}
	...
	map.minCost = DestroyPath(6,3,map.Edges.length,map.Edges,[1,2,4,0]);	
