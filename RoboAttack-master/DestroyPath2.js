...
function FindMin(u,v,w,NewGraph,p1,p2) {
	...
	deleteEdges.push([[u,v]]);	
	while(ptr1.parent != ptr1.data) {
		if(ptr1.weight < min) {
			min = ptr1.weight;
			data = ptr1.data;
			deleteEdges.pop();
			deleteEdges.push([[ptr1.data,ptr1.parent]]);
		} if(ptr1.weight == min) {
			deleteEdges.push(deleteEdges[deleteEdges.length-1].push([ptr1.data,ptr1.parent]));
		}
		ptr1 = ptr1.parent;
	}
	...
	while(ptr2.parent != ptr2.data) {
		if(ptr2.weight < min) {
			...
			j++;  /* if minimum weight edges is in set(containing v),then
				   * increase j by 1.*/
			deleteEdges.pop();
			deleteEdges.push([[ptr2.data,ptr2.parent]]);
		} if(ptr2.weight == min) {
			deleteEdges.push(deleteEdges[deleteEdges.length-1].push([ptr2.data,ptr2.parent]));
		}
		ptr2 = ptr2.parent;
	}
...
console.log("Edges to be deleted : " + deleteEdges);
return minTime;
}
